...
GO_TEST = GORACE="halt_on_error=1" go test $(BUILD_FLAG) $(BUILD_TAGS) -race -cover -coverprofile=cover.out -p 1
...
cover:
	@if test -f cover.out ; then go tool cover -html=cover.out; else echo "run make test first"; fi
