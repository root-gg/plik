...
<template>
  <aside class="w-full md:w-80 md:shrink-0 p-4 space-y-3 animate-slide-in">
    ...
    <!-- Notification Receivers (admin only) -->
    <div v-if="upload.admin && (upload.receivers?.length || upload.notifyCreator)" class="sidebar-section">
      <h3 class="text-xs font-semibold text-surface-400 uppercase tracking-wider mb-2">Notifications</h3>

      <div v-if="upload.notifyCreator" class="flex items-center gap-1.5 mb-2">
        ...
        <span class="text-xs text-surface-300">Creator will be notified</span>
      </div>

      <div v-if="upload.receivers?.length" class="flex flex-wrap gap-1">
        <span v-for="email in upload.receivers" :key="email"
